...
CLUSTER_FILE = "clusterdata/corr1.csv"
...
def analyseForYear(yearStr, monthStr="", dayStr=""):
    ...
    if monthStr != "":
        if dayStr != "":
            # query for day
            query = "SELECT sts.stations_id, AVG(vals.temperature_day) as avg_temp FROM air_temperature_values as vals LEFT JOIN air_temperature_stations as sts ON vals.stations_id = sts.stations_id WHERE vals.messdatum_date = date('" + yearStr + "-" + monthStr + "-" + dayStr + "') GROUP BY sts.stations_id"
        else:
            # query for one month
            daysInMonth = monthrange(int(yearStr), int(monthStr))[1]
            query = "SELECT sts.stations_id, AVG(vals.temperature_day) as avg_temp FROM air_temperature_values as vals LEFT JOIN air_temperature_stations as sts ON vals.stations_id = sts.stations_id WHERE vals.messdatum_date >= date('" + yearStr + "-" + monthStr + "-01') AND vals.messdatum_date <= date('" + yearStr + "-" + monthStr + "-" + str(daysInMonth) + "') GROUP BY sts.stations_id"
    else:
        # query for whole year
        query = "SELECT sts.stations_id, AVG(vals.temperature_day) as avg_temp FROM air_temperature_values as vals LEFT JOIN air_temperature_stations as sts ON vals.stations_id = sts.stations_id WHERE vals.messdatum_date >= date('" + yearStr + "-01-01') AND vals.messdatum_date <= date('" + yearStr + "-12-31') GROUP BY sts.stations_id"

    print(query)
    avgTemps = pd.read_sql_query(query, connection)

    # Load cluster data
    stationsCluster = pd.read_csv(CLUSTER_FILE)

    # Now join the cluster information with the temperature values, drop irrelevant cols
    joined = avgTemps.merge(stationsCluster, how='right')
    joined = joined.drop(['stations_id.1', 'stations_id'], axis=1)

    # Drop rows with NaN average temps
    ...
    # Add the results to the time series for each cluster
    for row in joined.iterrows():
        index, vals = row
        #name = vals['cluster']
        ...
        seriesIndex = seriesNames.index(index)
...
def plotAnalysis():
    ...
    plt.title("Durchschnittliche Temperatur in den Clustern [°C]")
    plt.legend(loc="lower right", fontsize="small")
    plt.show()

    plt.plot(xAxis, averagedSeries, label="Durchschnitt aller Cluster")
    ...
    plt.title("Veränderung der durchschnittl. Temp. in den Clustern [°C]")
...
# Now also calculate the min, max for change
for i in range(len(seriesChange[0])):
    vals = []
    for s in series:
        vals.append(s[i])

    minChangeSeries.append(min(vals))
    maxChangeSeries.append(max(vals))
