...
def analyseForYear(yearStr, monthStr="", dayStr=""):
    ...
    if monthStr != "":
        ...
        query = "SELECT sts.bundesland as bundesland, AVG(vals.temperature_day) as avg_temp FROM air_temperature_values as vals LEFT JOIN air_temperature_stations as sts ON vals.stations_id = sts.stations_id WHERE vals.messdatum_date >= date('" + yearStr + "-12-01') AND vals.messdatum_date <= date('" + str(int(yearStr) + 1) + "-02-28') GROUP BY sts.bundesland"
    ...
    if monthStr != "":
        ...
        # Just use the year
        xAxis.append(yearStr + "-" + str(int(yearStr) + 1))
...
def plotAnalysis():
    ...
    plt.title("Durchschnittliche Temperatur in den Bundesländern (Winter) [°C]")
    plt.legend(loc="lower right", fontsize="small")
    plt.show()

    plt.plot(xAxis, averagedSeries, label="Durchschnitt aller Bundesländer [°C]")
    ...
    plt.title("Aggergierte Temperatur aller Bundesländer (Winter) [°C]")
    plt.legend(loc="lower right", fontsize="small")
    plt.show()
...
plt.title("Ranking der höchsten durchschnittlichen Temperaturen von 2010 bis 2019 (Winter)")
plt.show()

# Plot as histograms
plt.bar(seriesNamesLosers, countLosers)
plt.title("Ranking der niedrigsten durchschnittlichen Temperaturen von 2010 bis 2019 (Winter)")
plt.show()
